...
let blogItems = document.querySelectorAll('.block-projects');
if (blogItems.length > 0) {
	let blogFilters = document.querySelectorAll('.projects__item');
	if (blogFilters.length > 0) {
		for (let index = 0; index < blogFilters.length; index++) {
			...
			blogFilter.addEventListener("click", function (e) {
				...
				const blogFilterActive = document.querySelector('.projects__item._active');
		...
		function showBlogItems(filter) {
			for (let index = 0; index < blogItems.length; index++) {
				...
				if (filter === 'all' || !filter) {
					...
					if (blogItem.classList.contains('block-projects_' + filter)) {
						blogItem.classList.remove('_hide');
					}
				}
			}
...
let slider_about = new Swiper('.main__slider', {
